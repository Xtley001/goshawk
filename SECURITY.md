...
Corvus enforces a strict separation of concerns across three distinct on-chain identities:

| Role | Storage Type | Permissions |
|---|---|---|
| `owner` | Multi-sig (Gnosis Safe) / Hardware Wallet | Whitelist protocols and routers, propose new executor, pause execution, sweep recovered tokens |
| `executor` | Server Hot EOA | Trigger `execute()` on whitelisted venues, call authorized flash loan callbacks |
...
| Arbitrary External Calls | All contract interactions require destination addresses to exist in `allowedProtocols` or `allowedRouters`. |
| Residual Token Approvals | All DEX and pool allowances are explicitly reset to zero immediately after swap operations. |
| Mempool Front-Running | Transactions route through private builder channels (`Flashbots`, `Timeboost`, `Bor Private`). |

## Reporting a Vulnerability

If you discover a security vulnerability in Corvus, report it privately. Do not open public GitHub issues, discussions, or pull requests.

- **Email:** `security@corvus.fi` (or via maintainer keybase / PGP)
